...
router.post("/comment/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    const isGoodText = predictionKeys.every((val) => predictionObj[val] < 80); //pred logic
    const post = await PostModel.findById(postId);
    if (!post) {
      return res.status(404).send("No Post found");
    }
    // direct re to predict api
    if (isGoodText) {
      const newComment = {
        _id: uuid(),
        text,
      ...
      const predictedLabels = predictionKeys
        .filter((i) => predictionObj[i] >= 80)
        .map((i) => normalizeText(i));
      const flags = predictedLabels.join(", ");
      return res
        .status(401)
        .send(
